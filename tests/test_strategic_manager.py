...
def test_destructive_action_is_blocked_by_policy():
    ...
    assert manager.evaluate(CityState(money=10_000), destructive).score == float("-inf")
...
def test_failed_verification_enters_recovery_and_audit():
    ...
    manager.record_execution(ActionResult(action=action, executed=True, verified=False,
