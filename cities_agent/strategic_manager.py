...
class StrategicManager:
    ...
    The manager diagnoses normalized state, creates low-risk candidates, evaluates
    them in an isolated MockCity copy, applies the safety policy, and records every
    decision. Real game I/O remains outside this class.
    """
    ...
    def candidates(self, state: CityState) -> list[Action]:
        actions: list[Action] = []
        # Only reversible, low-impact zoning is generated by this first strategy.
        # Roads/utilities/services require a richer simulator and verified geometry.
        if state.residential_demand is not None and state.residential_demand >= 40:
    ...
    def plan(self, state: CityState, simulator: MockCity | None = None) -> ManagerDecision:
        ...
        if goals and goals[0].type == GoalType.AVOID_FAILURE and goals[0].hard:
            ...
            return ManagerDecision(action, "A hard failure-avoidance condition is active; observe before changing the city.", [])
